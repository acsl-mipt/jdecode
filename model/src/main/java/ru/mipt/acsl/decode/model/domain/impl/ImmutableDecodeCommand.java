...
public class ImmutableDecodeCommand extends AbstractDecodeOptionalInfoAware implements DecodeCommand
{
    ...
    @NotNull
    public static DecodeCommand newInstance(@NotNull DecodeName name, int id, @NotNull Optional<String> info,
                                           @NotNull List<DecodeCommandArgument> arguments)
    {
        return new ImmutableDecodeCommand(name, id, info, arguments);
    }

    private ImmutableDecodeCommand(@NotNull DecodeName name, int id, @NotNull Optional<String> info,
                                   @NotNull List<DecodeCommandArgument> arguments)
    {
        super(info);
        this.name = name;
