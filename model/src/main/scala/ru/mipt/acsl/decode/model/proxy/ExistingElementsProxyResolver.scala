...
import ru.mipt.acsl.decode.model.{Message, Referenceable}
...
case object ExistingElementsProxyResolver extends DecodeProxyResolver {
  ...
  private def resolveLiteralElement(registry: Registry, literal: ProxyPath.Literal): ResolvingResult[Referenceable] = {
    ...
    if (elements.size == 1) {
      ...
      ResolvingResult.newInstance(obj, RegistryUtils.resolve(registry, obj))
    }
    else if (elements.size > 1)
      ResolvingResult.newInstance(Message.newError(s"must be exactly one element for $literal, found: ${elements.size}"))
    else
  ...
  private def resolveFqnElement(registry: Registry, fqnElement: ProxyPath.FqnElement): ResolvingResult[Referenceable] = {
    ...
    if (nsOption.isEmpty)
      return ResolvingResult.newInstance(Message.newError(s"namespace not found ${fqnElement.ns.mangledNameString()}"))
    val ns = nsOption.get
    fqnElement.element match {
      case e: TypeName =>
        ...
        if (elements.size == 1) {
          ...
          ResolvingResult.newInstance(Message.newError(s"must be exactly one element for $fqnElement, found: ${elements.size}"))
      case _ =>
        ResolvingResult.newInstance()
    }
  }
