...
class DecodeParboiledParser(val input: ParserInput) extends Parser with LazyLogging {
  ...
  private def unitForFqn(unitFqn: Fqn): MaybeProxy[DecodeUnit] = {
    if (unitFqn.size == 1 && imports.contains(unitFqn.asMangledString))
      // todo: remove asInstanceOf
      imports.getOrElse(unitFqn.asMangledString, sys.error("not found")).asInstanceOf[MaybeProxy[DecodeUnit]]
    else
  ...
  private def newTypeUnitApplication(t: Option[MaybeProxy[DecodeType]],
                                     unit: Option[MaybeProxy[DecodeUnit]]) =
    TypeUnit(t.get, unit)
  ...
  private def newCommandArg(info: Option[String], typeUnit: TypeUnit, name: ElementName) =
    Parameter(name, info, typeUnit.t, typeUnit.unit)
  ...
  private def newCommand(info: Option[String], name: ElementName, id: Option[Int],
    ...
    Command(name, id, info, args, returnType)

  def Command_ : Rule1[Command] = rule {
  ...
  private def newMessageParameter(info: Option[String], value: String) = MessageParameter(value, info)

  def MessageParameter_ : Rule1[MessageParameter] = rule {
  ...
  def MessageParameters: Rule1[Seq[MessageParameter]] = rule {
    '(' ~ Ew.? ~ MessageParameter_.+(Ew.? ~ ',' ~ Ew.?) ~ (Ew.? ~ ',').? ~ Ew.? ~ ')'
  }
  ...
  private def newEventMessage(info: Option[String], name: ElementName, id: Option[Int],
    ...
    EventMessage(component.get, name, id, info, parameters, baseType.get)

  def EventMessage_ : Rule1[EventMessage] = rule {
  ...
  private def newStatusMessage(info: Option[String], name: ElementName, id: Option[Int], priority: Option[Int],
    ...
    StatusMessage(component.get, name, id, info, parameters, priority)

  def StatusMessage_ : Rule1[StatusMessage] = rule {
  ...
  private def newComponent(info: Option[String], id: Option[Int], subComponents: Option[immutable.Seq[Fqn]],
    ...
    component = Some(Component(componentName.get, ns.get, id,
      ...
      if (fqn.size == 1 && imports.contains(alias)) {
        ...
        ComponentRef(MaybeProxy.proxyDefaultNamespace(fqn, ns.get), None)
  ...
  def Component_ : Rule1[Component] = rule {
    definition("component") ~> { name => componentName = Some(name) } ~ Id.? ~
      ...
      (Ew.? ~ (Command_ ~> { command => component.get.commands = component.get.commands :+ command }
      | (StatusMessage_ ~> { statusMessage => component.get.statusMessages = component.get.statusMessages :+ statusMessage }
        | EventMessage_ ~> { eventMessage => component.get.eventMessages = component.get.eventMessages :+ eventMessage }))).* ~
      Ew.? ~ '}'
  }

  private def newUnit(display: Option[String], name: ElementName, info: Option[String]) =
    DecodeUnit(name, ns.get, display, info)
  ...
  private def newEnumType(info: Option[String], name: ElementName, genericParameters: Option[Seq[ElementName]],
    ...
    EnumType(name, ns.get, t, info, constants.to[immutable.Set], isFinal)
  }
  ...
  private def newAliasType(info: Option[String], name: ElementName,
    ...
    AliasType(name, ns.get, typeAppOption.get, info)
  ...
  private def newLanguage(info: Option[String], name: ElementName, default: Boolean) =
    Language(name, ns.get, default, info)

  def Language_ : Rule1[Language] = rule {
        ...
        ns.types = ns.types :+ Option(body.getEnumTypeDecl).map{ e =>
          EnumType(name, ns, Option(e.getElementNameRule)
            ...
            e.getEnumTypeValues.getEnumTypeValueList.map{ v => EnumConstant(elementName(v.getElementNameRule),
              v.getLiteral.getText, optionString(v.getInfoString))}.to[immutable.Set], e.getFinalEnum != null)
        }.orElse{
          Option(body.getNativeTypeDecl).map(n =>
            Option(p.getGenericArgs).map(args => GenericType(name, ns, optionInfo,
              args.getGenericArgList.map(arg => Option(arg.getElementNameRule).map(elementName(_)))))
...
class DecodeSourceProvider extends LazyLogging {
  ...
  def provideOld(config: DecodeSourceProviderConfiguration): Registry = {
    val resourcePath = config.resourcePath
    val registry = Registry()
    val resourcesAsStream = getClass.getResourceAsStream(resourcePath)
    require(resourcesAsStream != null, resourcePath)
